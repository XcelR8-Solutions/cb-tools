...
##
# Splunk API Check : Build and make call
##
CHECK_NAME="Splunk API Check [$SERVER_NAME]"
CHECK_URL="https://$EXTERNAL_IP:8089/services/server/info"
...
curl -k -XPOST "$MORPHEUS_URL/api/monitoring/checks" \
  ...
  -d "{'check':{
    ...
    'config': {
      'checkUser':'admin',
      'checkPassword':'n+fY%bqb3nbwBnbx',
      'webMethod': 'GET',
      'webUrl': '$CHECK_URL'
    }
  }}"